 ...
 * 추가된 것만 확인하고 신규 내역을 직접 엘리먼트로 작성해 추가하도록 하면 서버에 요청을 두 번씩 보낼 필요가 없어짐. 세션 정보는 서버에
 * 저장되어 있으므로 ajax로 사용자 id를 받아올 수 있게 하는 기능을 추가해야 한다.
 */
function sendReservation(placeName) {
	...
	var url = "/addReservation.do?placeName=" + placeName + "&date="
			+ resvForm.date.value + "&startTime=" + resvForm.startTime.value
			+ "&endTime=" + resvForm.endTime.value + "&purpose="
